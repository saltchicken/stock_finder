...
class FinvizClient:
    ...
    def get_screener_tickers(self, filters: list) -> list:
        ...
        try:
            while True:
                logger.info(f"Fetching screener results {page_start} to {page_start + 19}... (Total found so far: {len(tickers)})")
                ...
                seen = set()
                unique_tickers = [x for x in page_tickers if not (x in seen or seen.add(x))]
